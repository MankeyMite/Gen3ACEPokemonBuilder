...
const lines = [
  ...
  '// Types: l=land(12), w=water(5), r=rock_smash(5), o=old_rod(2), g=good_rod(3), s=super_rod(5)',
...
for (const gId of sortedGames) {
  lines.push(`  ${gId}:{`);
  const sortedLocs = Object.keys(result[gId]).map(Number).sort((a, b) => a - b);
