...
// speciesId → { levelUp: [[moveId, level], ...], egg: Set, tm: Set, tutor: Set, xdTutor: Set }
const learnsets = {};

function ensureEntry(speciesId) {
  if (!learnsets[speciesId]) {
    learnsets[speciesId] = { levelUp: [], egg: new Set(), tm: new Set(), tutor: new Set(), xdTutor: new Set() };
  }
...
// PKHeX stores these arrays as sorted National Pokédex species IDs.
{
  ...
  for (const [moveConst, nationalSpecies] of Object.entries(xdTutorSpeciesByMove)) {
    ...
    for (const nationalDex of nationalSpecies) {
      const entry = ensureEntry(nationalToInternalSpeciesId(nationalDex));
      entry.tutor.add(moveId);
      entry.xdTutor.add(moveId);
    }
  }
...
const lines = [
  ...
  '// Maps speciesId to merged/direct level-up, egg, TM/HM, tutor, and XD-only tutor moves.',
...
for (const sid of sortedSpeciesIds) {
  ...
  const xdTutor = [...e.xdTutor].sort((a, b) => a - b);
  ...
  const xdTutorStr = xdTutor.join(',');
  totalEntries += lu.length + egg.length + tm.length + tutor.length;

  lines.push(`  ${sid}: {l:[${luStr}],d:[${directLuStr}],e:[${eggStr}],t:[${tmStr}],u:[${tutorStr}],x:[${xdTutorStr}]}, // ${name}`);
}
