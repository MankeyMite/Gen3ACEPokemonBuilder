...
const emeraldDirectLearnsets = {};
const emeraldEntryPattern = /^\s*(\d+):\s*\{l:\[(.*?)\],d:\[(.*?)\],e:/gm;
let emeraldMatch;
while ((emeraldMatch = emeraldEntryPattern.exec(emeraldGeneratedSource)) !== null) {
  const parseGeneratedMoves = body => {
    const moves = [];
  ...
  const speciesId = Number(emeraldMatch[1]);
  emeraldLearnsets[speciesId] = parseGeneratedMoves(emeraldMatch[2]);
  emeraldDirectLearnsets[speciesId] = parseGeneratedMoves(emeraldMatch[3]);
}
...
const cloneLearnsets = learnsets => Object.fromEntries(
  Object.entries(learnsets).map(([speciesId, moves]) => [speciesId, moves.map(move => [...move])])
);
const rsDirectLearnsets = cloneLearnsets(rsLearnsets);
const fireRedDirectLearnsets = cloneLearnsets(fireRedLearnsets);
const leafGreenDirectLearnsets = cloneLearnsets(leafGreenLearnsets);
mergePreEvolutionMoves(rsLearnsets);
mergePreEvolutionMoves(fireRedLearnsets);
mergePreEvolutionMoves(leafGreenLearnsets);

function getOverrides(learnsets, baselineLearnsets, gameLabel) {
  const overrides = {};
  for (const [speciesIdText, moves] of Object.entries(learnsets)) {
    ...
    const emerald = normalizeMoves(baselineLearnsets[speciesId] || []);
...
writeOverrides('learnsets.rs.js', [
  ['LEARNSETS_RS', getOverrides(rsLearnsets, emeraldLearnsets, 'Ruby/Sapphire merged')],
  ['LEARNSETS_RS_DIRECT', getOverrides(rsDirectLearnsets, emeraldDirectLearnsets, 'Ruby/Sapphire direct')],
]);
writeOverrides('learnsets.frlg.js', [
  ['LEARNSETS_FIRE_RED', getOverrides(fireRedLearnsets, emeraldLearnsets, 'FireRed merged')],
  ['LEARNSETS_LEAF_GREEN', getOverrides(leafGreenLearnsets, emeraldLearnsets, 'LeafGreen merged')],
  ['LEARNSETS_FIRE_RED_DIRECT', getOverrides(fireRedDirectLearnsets, emeraldDirectLearnsets, 'FireRed direct')],
  ['LEARNSETS_LEAF_GREEN_DIRECT', getOverrides(leafGreenDirectLearnsets, emeraldDirectLearnsets, 'LeafGreen direct')],
]);
